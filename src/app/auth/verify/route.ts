...
/**
 * This route handles Supabase's default invite links and redirects them
 * to our clean domain format. This allows the Supabase email template
 * to use {{ .ConfirmationURL }} but still redirect through our domain.
 */
export async function GET(req: Request) {
  try {
    ...
    if (data?.user) {
      ...
      return NextResponse.redirect(new URL('/auth/setup-password', siteUrl));
    }
